...
class HiSparseCoordinator:
    ...
    def write_newest_rows(
        ...
        # (and with it the whole DP fleet). Real rows are requests x
        # tokens_per_req; padded hot-slot writes are unobservable (the swap-in
        # kernel skips rows past num_real_rows and matches newest slots per
        # request).
        num_tokens = min(
            kv_c_normed.shape[0],
            slot_mapping.numel(),
            self.max_num_reqs * tokens_per_req,
        )
